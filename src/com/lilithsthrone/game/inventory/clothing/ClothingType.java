...
//	};
	...
	public static AbstractClothingType FINGER_LYSSIETHS_RING = new AbstractClothingType(50000,
		...
		public String unequipText(GameCharacter clothingOwner, GameCharacter clothingRemover, InventorySlot slotToEquipInto, boolean rough, AbstractClothing clothing, boolean applyEffects) {
			if(rough) {
                return UtilText.parse(clothingRemover, clothingOwner, "[npc.Name] roughly yank the signet ring off of [npc2.namePos] finger.");
			} else {
				if(clothingOwner.equals(clothingRemover)) {
                    return UtilText.parse(clothingRemover, clothingOwner, "[npc.Name] slide the signet ring off [npc.her] finger.");
				} else {
                    return UtilText.parse(clothingRemover, clothingOwner, "[npc.Name] slide the signet ring off [npc2.namePos] finger.");
				}
			}
		}
	};
