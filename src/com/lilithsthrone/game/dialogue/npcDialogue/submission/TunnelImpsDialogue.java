...
public class TunnelImpsDialogue {
	...
	public static final DialogueNode AFTER_SEX_DEFEAT = new DialogueNode("Collapse", "", true) {
		...
		public Response getResponse(int responseTab, int index) {
			if(getImpGroup().isEmpty()) {
				if(index==1) {
                    return new Response("Продолжить", "As you've enslaved all of the imps, there's nothing left to do but continue on your way...", Main.game.getDefaultDialogue(false));
				}
				return null;
			}
			if(!isCompanionDialogue()) {
				if(responseTab == 0) {
					if (index == 1) {
                        return new Response("Продолжить", "Leave the imps and continue on your way...", Main.game.getDefaultDialogue(false)) {
							@Override
							public void effects() {
