...
public class DominionExpress {
	...
	private static final List<MuleReward> muleRewards;
	static {
	...
	public static final DialogueNode FILLY_STATION_REWARD = new DialogueNode("", "", true) {
		...
		public Response getResponse(int responseTab, int index) {
			if(index==1) {
                return new Response("Продолжить", "Continue interacting with the [style.mule] reward station.", FILLY_STATION_MACHINE);
			}
			return null;
		}
	};
		...
		public Response getResponse(int responseTab, int index) {
			if(index==1) {
				if(Main.game.getPlayer().isQuestCompleted(QuestLine.ROMANCE_NATALYA)) {
                    ...
                    return new Response("Вход", "As you are not a slave or qualified [style.mule], you cannot enter the stables...", null);
				}
			}
			return null;
		}
	...
	public static final DialogueNode OFFICE_STABLE_CONTRACT_SIGNED = new DialogueNode("", "", true, true) {
		...
		public Response getResponse(int responseTab, int index) {
			if(isPlayerBodyCorrect()) {
				...
				if(index==1) {
					return new Response("Выпить potion", "Выпить the potion and be transformed into one of Mistress Natalya's [style.shemale] [style.mules].", OFFICE_STABLE_TRANSFORMED) {
						@Override
						public Colour getHighlightColour() {
							return PresetColour.TRANSFORMATION_GENERIC;
						}
