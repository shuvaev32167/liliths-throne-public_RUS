    ...
    private void setUpButtons() {
        ...
        actionKeyPressed = new EventHandler<KeyEvent>() {
            ...
            private void printAlreadyExistingBinding(String primarySecondary, String actionName, String eventCodeName) {
                Main.game.getTextStartStringBuilder().append("<p style='text-align:center;'>" + "<b style='color:" + PresetColour.GENERIC_BAD.toWebHexString() + ";'>Клавиша '" + eventCodeName
                        + "' уже " + primarySecondary + " назначена на '" + actionName + "'!</b>" + "</p>");
                Main.game.setContent(new Response("", "", Main.game.getCurrentDialogueNode()));
            }
                            ...
                            if ((boolean) Main.mainController.getWebEngine().executeScript("document.getElementById('nameInput') === document.activeElement")) {
                                ...
                                if (event.getCode() == KeyCode.ENTER) {
                                    ...
                                    if (Main.mainController.getWebEngine().executeScript("document.getElementById('nameInput')") != null) {
                                        ...
                                        if (!unsuitableName) {
                                            Main.game.setContent(new Response("Переименовать комнату", "Переименовать эту комнату так, как вы указали в текстовом поле.", Main.game.getCurrentDialogueNode()) {
                                                @Override
                                                public void effects() {
                                            ...
                                            Main.game.setContent(new Response("Переименовать комнату", "", Main.game.getCurrentDialogueNode()));
                                        }
                                    }
                                }
                            }
                        ...
                        if (Main.game.getCurrentDialogueNode() == OptionsDialogue.SAVE_LOAD) {
                            if ((boolean) Main.mainController.getWebEngine().executeScript("document.getElementById('new_save_name') === document.activeElement")) {
                                ...
                                if (event.getCode() == KeyCode.ENTER) {
                                    ...
                                    Main.game.setContent(new Response("Сохранить", "", Main.game.getCurrentDialogueNode()));
                                }
                            }
                        }
                                ...
                                for (GenderPronoun gp : GenderPronoun.values()) {
                                    if ((boolean) Main.mainController.getWebEngine().executeScript("document.getElementById('feminine_" + gp + "') === document.activeElement")
                                        ...
                                        Main.game.setContent(
                                                new Response("Сохранить", "Сохранить все местоимения, которые отображаются в данный момент.", OptionsDialogue.OPTIONS_PRONOUNS) {
                                                    @Override
                                                    public void effects() {
...
//		webEngine.setJavaScriptEnabled(false);
            ...
            for (int i = 0; i < DicePoker.getPlayerDice().size(); i++) {
                ...
                if ((document.getElementById(id)) != null) {
                    ...
                    addTooltipListeners(id, new TooltipInformationEventListener().setInformation(
                            "Отметить для повторного броска",
                            DicePoker.isAbleToSelectReroll() ? "" : "[style.italicsBad(В этой сцене вы не можете пометить свои кубики для повторного броска!)]"));
                }
            }
        }
    ...
    private void manageButtonLeftListeners() {
        ...
        if (documentButtonsLeft.getElementById("mainMenu") != null) {
            ...
            addEventListener(documentButtonsLeft, "mainMenu", "mousemove", moveTooltipListener, false);
            addEventListener(documentButtonsLeft, "mainMenu", "mouseleave", hideTooltipListener, false);
            hotKey = Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.MENU);
            addEventListener(documentButtonsLeft, "mainMenu", "mouseenter", new TooltipInformationEventListener().setInformation("Главное меню" + (hotKey == null ? "" : " (" + hotKey.getFullName() + ")"), ""), false);
        }
        if (documentButtonsLeft.getElementById("inventory") != null) {
            ...
            addEventListener(documentButtonsLeft, "inventory", "mousemove", moveTooltipListener, false);
            addEventListener(documentButtonsLeft, "inventory", "mouseleave", hideTooltipListener, false);
            hotKey = Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.INVENTORY);
            addEventListener(documentButtonsLeft, "inventory", "mouseenter", new TooltipInformationEventListener().setInformation("Инвентарь" + (hotKey == null ? "" : " (" + hotKey.getFullName() + ")"), ""), false);
        }
        if (documentButtonsLeft.getElementById("journal") != null) {
            ...
            addEventListener(documentButtonsLeft, "journal", "mousemove", moveTooltipListener, false);
            addEventListener(documentButtonsLeft, "journal", "mouseleave", hideTooltipListener, false);
            hotKey = Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.JOURNAL);
            addEventListener(documentButtonsLeft, "journal", "mouseenter", new TooltipInformationEventListener().setInformation("Телефон" + (hotKey == null ? "" : " (" + hotKey.getFullName() + ")"), ""), false);
        }
        if (documentButtonsLeft.getElementById("charactersPresent") != null) {
            ...
            addEventListener(documentButtonsLeft, "charactersPresent", "mousemove", moveTooltipListener, false);
            addEventListener(documentButtonsLeft, "charactersPresent", "mouseleave", hideTooltipListener, false);
            hotKey = Main.getProperties().hotkeyMapPrimary.get(KeyboardAction.CHARACTERS);
            addEventListener(documentButtonsLeft, "charactersPresent", "mouseenter", new TooltipInformationEventListener().setInformation("Присутствующие персонажи" + (hotKey == null ? "" : " (" + hotKey.getFullName() + ")"), ""), false);
        }
        if (documentButtonsLeft.getElementById("mapZoom") != null) {
            ...
            addEventListener(documentButtonsLeft, "mapZoom", "mousemove", moveTooltipListener, false);
            addEventListener(documentButtonsLeft, "mapZoom", "mouseleave", hideTooltipListener, false);
    ...
    private void manageButtonRightListeners() {
        ...
        if (documentButtonsRight.getElementById("quickSave") != null) {
            ...
            addEventListener(documentButtonsRight, "quickSave", "mouseenter", new TooltipInformationEventListener().setInformation(
                            ...
                            "Либо создает, либо " + (quickLoadAvailable ? "[style.italicsBad(перезаписывает)]" : "") + " файл сохранения с именем «" + Main.getQuickSaveName() + "»."
                                    + (hotKey == null
                                    ? "<br/>В настоящее время к этому действию не привязано ни одно сочетание клавиш."
                                    : "<br/>Вы также можете быстро сохранить, нажав клавишу «" + hotKey.getFullName() + "».")
                                    + (quickSaveAvailable
                                    ? "<br/>[style.italicsGood(В этой сцене можно сохранить игру!)]"
        ...
        if (documentButtonsRight.getElementById("quickLoad") != null) {
            ...
            addEventListener(documentButtonsRight, "quickLoad", "mouseenter", new TooltipInformationEventListener().setInformation(
                            ...
                            "Загружает файл сохранения с именем «" + Main.getQuickSaveName() + "»."
                                    + (hotKey == null
                                    ? "<br/>В настоящее время к этому действию не привязано ни одно сочетание клавиш."
                                    : "<br/>Также можно выполнить быструю загрузку, нажав клавишу «" + hotKey.getFullName() + "».")
                                    + (quickLoadAvailable
                                    ? "<br/>[style.italicsGood(Для этого персонажа обнаружен файл быстрого сохранения!)]"
                                    : "<br/>[style.italicsBad(Файл быстрого сохранения для этого персонажа не обнаружен!)]")),
                    false);
        }
        ...
        if (documentButtonsRight.getElementById(id) != null) {
            ...
            addEventListener(documentButtonsRight, id, "mouseenter", new TooltipInformationEventListener().setInformation(
                    ...
                    "Нажмите, чтобы скопировать отображаемый диалог в буфер обмена. Вы можете вставить полученный результат в текстовый редактор, сохранить его как файл <i>.html</i>, а затем открыть этот файл в браузере, чтобы сохранить форматирование игры.<br/>"
                            + "Эта сцена написана: <b>" + Main.game.getCurrentDialogueNode().getAuthor() + "</b>",
                    120), false);
        ...
        if (documentButtonsRight.getElementById(id) != null) {
            ...
            if (exportAvailable) {
                ...
                addEventListener(documentButtonsRight, id, "mouseenter", new TooltipInformationEventListener().setInformation(
                        "Экспорт персонажа",
                        "Чтобы экспортировать любого персонажа, включая вашего собственного, вам необходимо просмотреть его на экране «Персонажи присутствуют» на вашем телефоне.",
                        48), false);
            }
        }
    }
	...
	private void manageAttributeListeners() {
        ...
        if (documentAttributes.getElementById(id) != null) {
            ...
            ((EventTarget) documentAttributes.getElementById(id)).addEventListener("mousedown", e -> {
                if (!Main.game.isBadEnd() && e instanceof MouseEventImpl mouseEvent && mouseEvent.getButton() == 2) {
                    Main.game.setSecondsPassed(Main.game.getSecondsPassed() - 3_600);
                    updateUI();
                }
            }, false);
            ...
            addEventListener(documentAttributes, id, "mousemove", moveTooltipListener, false);
            addEventListener(documentAttributes, id, "mouseleave", hideTooltipListener, false);
            String day = Main.game.getDateNow().getDayOfWeek().getDisplayName(TextStyle.FULL, RUSSIAN_LOCALE);
            StringBuilder dateSB = new StringBuilder();
            if (Main.game.isBadEnd()) {
                dateSB.append("[style.italicsBad(Учитывая текущую ситуацию, ты [pc.genderBasedWord(потерял, потеряла)] счёт тому, какой сегодня день...)]");
            } else {
                if (!dateKnown) {
                    dateSB.append("[style.italicsMinorBad(Посмотри на календарь в своей комнате, чтобы узнать дату!)]");
                } else {
                    dateSB.append("Текущая дата: [style.colourGood(" + Main.game.getDisplayDate(true) + ")]");
                }
                dateSB.append("<br/>");
                dateSB.append("Сегодня " + UtilText.generateSingularDeterminer(day) + " [style.colourBlueLight(" + day + ")].<br/>"
                        + "Ты [pc.genderBasedWord(был, была)] в этом новом мире уже: [style.colourExcellent(" + Main.game.getDayNumber() + " " + Morpher.morphCountableNoun(Main.game.getDayNumber(), "день") + ")]");
                dateSB.append("<br/><i>Нажмите, чтобы переключиться между датой и количеством дней.</i>");
            }
            TooltipInformationEventListener el2 = new TooltipInformationEventListener().setInformation("Переключить отображение календаря",
                    dateSB.toString());
            addEventListener(documentAttributes, id, "mouseenter", el2, false);
        }
        ...
        if (documentAttributes.getElementById(id) != null) {
            ...
            if (Main.game.isBadEnd()) {
                timeSB.append("[style.italicsBad(Учитывая текущую ситуацию, ты [pc.genderBasedWord(потерял, потеряла)] счёт времени...)]");
            } else {
                timeSB.append("Текущее время: " + Units.time(Main.game.getDateNow()) + " (<span style='color:" + dp.getColour().toWebHexString() + ";'>" + Util.capitaliseSentence(dp.getName()) + "</span>)<br/>"
                        + "Восход: " + (!dateKnown ? "[style.colourMinorBad(??/??/??)] [style.colourYellow(" + Units.time(ldt[0]) : "[style.colourYellow(" + Units.dateTime(ldt[0])) + ")]<br/>"
                        + "Закат: " + (!dateKnown ? "[style.colourMinorBad(??/??/??)] [style.colourOrange(" + Units.time(ldt[1]) : "[style.colourOrange(" + Units.dateTime(ldt[1])) + ")]<br/>"
                        + "<i>Нажмите, чтобы переключить отображение времени между 24-часовым и 12-часовым форматом.</i>");
            }
            TooltipInformationEventListener el2 = new TooltipInformationEventListener().setInformation(
                    (Main.game.isDayTime() ? "День" : "Ночь"),
                    timeSB.toString());
            addEventListener(documentAttributes, id, "mouseenter", el2, false);
        }
        ...
        if (documentAttributes.getElementById(id) != null) {
            ...
            TooltipInformationEventListener el2 = new TooltipInformationEventListener().setInformation("[style.boldArcane(Магические эссенции)]", "");
            addEventListener(documentAttributes, id, "mouseenter", el2, false);
        }
        ...
        if (lastKeysEqual(KeyCode.B, KeyCode.U, KeyCode.G, KeyCode.G, KeyCode.Y)) {
            if (Main.game != null) {
                if (Main.game.isStarted() && Main.game.isInNewWorld() && Main.game.isPrologueFinished()) {
                    ...
                    Main.game.flashMessage(PresetColour.GENERIC_BAD, "Недоступно в прологе!");
                }
            }
        }
...
//					Main.game.setContent(new Response("", "", Main.game.getDefaultDialogue(false)));
                    ...
                    Main.game.flashMessage(PresetColour.GENERIC_BAD, "Невозможно доминировать!");
                }
            }
        }
        ...
        if (location.getY() + yOffset < Main.game.getActiveWorld().WORLD_HEIGHT
            ...
            if (!placeTypeTarget.getPlace().getPlaceType().equals(PlaceType.GENERIC_IMPASSABLE)) {
                if (Main.game.isInGlobalMap() && placeTypeTarget.getDialogue(false) == null) {
                    Main.game.flashMessage(PresetColour.GENERIC_BAD, "Невозможно путешествовать сюда!");
